import { NextResponse } from "next/server";
import { createClient as createSupabaseClient } from "@supabase/supabase-js";
import { productSchema, parseError } from "@/lib/validation";
import { hasEnvVars } from "@/lib/utils";
import type { Product } from "@/lib/data/products";

export async function POST(request: Request) {
  let body: unknown;
  try {
    body = await request.json();
  } catch {
    return NextResponse.json(
      { ok: false, error: "Invalid request body" },
      { status: 400 },
    );
  }

  const parsed = productSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      { ok: false, error: "Please fix the highlighted fields", fields: parseError(parsed.error) },
      { status: 400 },
    );
  }
  const input = parsed.data;

  const image = input.image || "/images/product/product-two-ssauto.jpeg";

  if (hasEnvVars) {
    try {
      const supabase = createSupabaseClient(
        process.env.NEXT_PUBLIC_SUPABASE_URL!,
        process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY!,
        { auth: { persistSession: false, autoRefreshToken: false } },
      );
      const { data, error } = await supabase
        .from("products")
        .insert({
          sku: input.sku,
          name: input.name,
          category: input.category,
          badge: input.badge,
          condition: input.condition,
          image,
          vehicle_compatibility: input.vehicleCompatibility,
          price: input.price,
          stock_status: input.stockStatus,
          description: input.description,
        })
        .select("*")
        .single();

      if (error) {
        const hint =
          error.code === "42P01"
            ? "The products table does not exist yet — run the migration SQL in the Supabase SQL editor first."
            : error.code === "23505"
              ? `A part with SKU ${input.sku} already exists.`
              : "The part could not be saved to the database.";
        return NextResponse.json(
          { ok: false, error: hint, code: error.code },
          { status: 500 },
        );
      }

      const product: Product = {
        id: data.id,
        sku: data.sku,
        name: data.name,
        category: data.category,
        badge: data.badge,
        condition: data.condition,
        image: data.image,
        vehicleCompatibility: data.vehicle_compatibility,
        price: data.price,
        stockStatus: data.stock_status,
        description: data.description,
      };
      return NextResponse.json({ ok: true, product });
    } catch {
      return NextResponse.json(
        { ok: false, error: "Could not reach the database. Please try again." },
        { status: 500 },
      );
    }
  }

  // No env vars: dev/demo mode — return the created product without persisting.
  const simulated: Product = {
    id: `local-${crypto.randomUUID()}`,
    ...input,
    image,
  };
  return NextResponse.json({ ok: true, product: simulated, demo: true });
}